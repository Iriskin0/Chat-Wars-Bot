...
Работающие функции бота:
  ...
  - донат в лавку вместо казны (покупка-продажа барахла) (по-умолчанию выключен, также должен быть включен обычный донат)
...
Как запустить:<br />
  ...
  5) Качаем этот скрипт и запускаем: `python3 ./main.py --admin "ваш ник" --order "ник игроков/ботов, выдающих приказы, через запятую   без пробелов" --castle "blue/red/black/white/yellow" --gold "сколько золота оставлять в кармане при автодонате" --buy "любое значение для включение траты денег в лавке покупкой/продажей барахла вместо доната в казну"`<br />
